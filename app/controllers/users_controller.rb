...
class UsersController < ApplicationController
  ...
  def create_second_factor
    ...
    unless current_user.confirm_password?(params[:password])
      return render json: failed_json.merge(
        error: I18n.t("login.incorrect_username_email_or_password")
      )
    end
  ...
  def update_second_factor
    ...
    unless current_user.authenticate_totp(params[:token])
      return render json: failed_json.merge(
        error: I18n.t("login.invalid_second_factor_code")
      )
    end
    ...
    if params[:enable] == "true"
      current_user.create_totp(enabled: true)
    else
      current_user.user_second_factor.destroy!

class PostRevisionSerializer < ApplicationSerializer
  ...
  def include_tags_changes?
    SiteSetting.tagging_enabled && previous["tags"] != current["tags"] && (!topic.private_message? || scope.user&.staff?)
  end
  ...
  protected
    ...
    def all_revisions
      ...
      if SiteSetting.tagging_enabled && (!topic.private_message? || scope.user&.staff?)
